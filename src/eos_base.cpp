...
double EOS_base::get_T2e_finite_rhob(const double T, const double rhob) const {
    double T_goal = T/HBARC;         // convert to 1/fm
    ...
    if (T_goal < 0.0 || T_goal > T_upper) {
        std::cout << "get_T2e:: T is out of bound, "
             << "T = " << T << " GeV, T_upper = " << T_upper*HBARC
             << ", T_lower = " << T_lower*HBARC << std::endl;
        exit(1);
    }
    ...
    if (iter == ntol) {
        std::cout << "get_T2e_finite_rhob:: max iteration reached, "
             << "T = " << T << " GeV, rhob = " << rhob << std::endl;;
        std::cout << "T_upper = " << T_upper*HBARC
             << " , T_lower = " << T_lower*HBARC << std::endl;
        std::cout << "eps_upper = " << eps_upper
             << " , eps_lower = " << eps_lower
             << ", diff = " << (eps_upper - eps_lower) << std::endl;
        exit(1);
    }
    return (eps_mid);
}
...
double EOS_base::get_s2e_finite_rhob(double s, double rhob) const {
    ...
    int ntol         = 1000;
    if (s < 0.0 || s > s_upper) {
        std::cout << "get_s2e_finite_rhob:: s is out of bound, "
             << "s = " << s << ", s_upper = " << s_upper
             << ", s_lower = " << s_lower << std::endl;
        exit(1);
    }
    ...
    if (iter == ntol) {
        std::cout << "get_s2e_finite_rhob:: max iteration reached, "
             << "s = " << s << ", rhob = " << rhob << std::endl;
        std::cout << "s_upper = " << get_entropy(eps_upper, rhob)
             << " , s_lower = " << get_entropy(eps_lower, rhob) << std::endl;
        std::cout << "eps_upper = " << eps_upper
             << " , eps_lower = " << eps_lower
             << ", diff = " << (eps_upper - eps_lower) << std::endl;
        exit(1);
...
void EOS_base::map_TmuB2erhoB(const double T, const double muB,
    ...
    const double muB_goal = muB/HBARC;     // convert to 1/fm
    double rhob_lower = 0.;
    ...
    if (muB_goal < 0.0 || muB_goal > muB_upper) {
        std::cout << "map_TmuB2erhoB:: muB is out of bound, "
             << "muB = " << muB << " GeV, muB_upper = "
             << muB_upper*HBARC << " GeV, muB_lower = "
             << muB_lower*HBARC << std::endl;
        exit(1);
    }
    ...
    if (iter == ntol) {
        std::cout << __PRETTY_FUNCTION__ << ":: max iteration reached, "
             << "T = " << T << " GeV, muB = " << muB << " GeV" << std::endl;;
        std::cout << "muB_upper = " << muB_upper*HBARC
             << " GeV, muB_lower = " << muB_lower*HBARC << std::endl;
        std::cout << "rhob_upper = " << rhob_upper
             << " , rhob_lower = " << rhob_lower
             << ", diff = " << (rhob_upper - rhob_lower) << std::endl;
        exit(1);
    }
    e = e_mid;
    rhob = rhob_mid;
...
void EOS_base::check_eos_no_muB() const {
    ...
    check_file << "#e(GeV/fm^3) P(GeV/fm^3) s(1/fm^3) T(GeV) cs^2" << std::endl;
    double e0 = 5e-4;
    double emax = 100.;
    double de = 5e-3;
    int ne = (emax - e0)/de + 1;
    ...
    check_file1 << "#e(GeV/fm^3) P(GeV/fm^3) s(1/fm^3) T(GeV) cs^2" << std::endl;
    e0 = 1e-8;
    emax = 1e-1;
    de = 2;
...
void EOS_base::check_eos_with_finite_muB() const {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        check_file << "#e(GeV/fm^3)  P(GeV/fm^3)  s(1/fm^3)  T(GeV)  cs^2  "
                   << "mu_B(GeV)  mu_S(GeV)  mu_C(GeV)" << std::endl;
        double e0 = 5e-4;
        double emax = 100.;
        double de = 5e-3;
        int ne = (emax - e0)/de + 1;
    ...
    for (int i = 0; i < ne; i++) {
        ...
        check_file1 << std::endl;
        check_file2 << std::endl;
    }
    check_file1.close();
    check_file2.close();
...
void EOS_base::outputMutable() const {
    ...
    check_file9 << "# e(GeV/fm^3)  rho_B(1/fm^3)  P (GeV/fm^3)  S(1/fm^3)  "
                << "T(GeV)  mu_B(GeV)  mu_S(GeV)  mu_Q(GeV)" << std::endl;
    for (int j = 0; j < ne; j++) {

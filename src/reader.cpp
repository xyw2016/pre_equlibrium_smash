     ...
     while(fin.good()){
      ...
      while(fin.read(reinterpret_cast<char*>(&fnptc), sizeof(float))){
        ...
        if(nptc0==888888){
          ...
          for (int ivec = 0 ; ivec < ptcl_tau.size();ivec++){
            ptcl_tau[ivec].clear();  
          }
        ...
        if(nptc0==999999){
          ptcl_event.push_back(ptcl_tau);
          
          for (int ivec = 0 ; ivec < ptcl_tau.size();ivec++)
          {
            std::cout<<"PTC number: "<<  ptcl_tau[ivec].size()<<std::endl;    
          }
        ...
        int index_taui = static_cast<int>((particle0.tau-dtau_eps)/dtau);
        ptcl_tau[index_taui].push_back(particle0);
...
//             if (particle0.tau < Parameter::TAU0-1e-6){
//               continue;
//             }
